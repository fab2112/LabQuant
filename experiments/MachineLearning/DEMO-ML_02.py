# LabQuant with Machine Learning - Demo2 - RandomForestClassifier - Monte Carlo test
import os
import pickle
import numpy as np
import pandas as pd
...
def build_model():
    ...
    pickle.dump(model, open("./Saved_models/rf_model_classifier_02.sav", "wb"))
...
# Load model
model = pickle.load(open("./Saved_models/rf_model_classifier_02.sav", "rb"))
...
def strategy(args):
    ...
    # Set target
    time_ref = 12
    target = df.c.pct_change(time_ref).shift(-time_ref)
    pct_ref = 5 / 100
    df.target = np.where(target >= pct_ref, 1, np.where(target < -pct_ref, 0, np.nan))
